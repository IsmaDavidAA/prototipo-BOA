import React from "react";
import QRCode from "react-qr-code";


const PagoQR = () =>{
const descargar = () =>{
    return(
        alert("Tu codigo sera descargado pronto, gracias por pagar!!!")
    )
}
return(
    <div class="card p-2 m-3 border-0">
        <div class="card-body ">
            <div class="d-inline d-flex justify-content-center m-2">
                <QRCode value="Pago QR" />
                    <button type="button" class="btn m-3" data-toggle="tooltip" data-placement="bottom" title="En la apliacion de su banco escanee el codigo o pulse el boton para descargarlo">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-question-circle" viewBox="0 0 16 16">
                            <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                            <path d="M5.255 5.786a.237.237 0 0 0 .241.247h.825c.138 0 .248-.113.266-.25.09-.656.54-1.134 1.342-1.134.686 0 1.314.343 1.314 1.168 0 .635-.374.927-.965 1.371-.673.489-1.206 1.06-1.168 1.987l.003.217a.25.25 0 0 0 .25.246h.811a.25.25 0 0 0 .25-.25v-.105c0-.718.273-.927 1.01-1.486.609-.463 1.244-.977 1.244-2.056 0-1.511-1.276-2.241-2.673-2.241-1.267 0-2.655.59-2.75 2.286zm1.557 5.763c0 .533.425.927 1.01.927.609 0 1.028-.394 1.028-.927 0-.552-.42-.94-1.029-.94-.584 0-1.009.388-1.009.94z"/>
                        </svg>
                    </button>
            </div>
            <div class="d-inline d-flex justify-content-center p-2" onClick ={ ()=>descargar()} >
               <button class="btn btn-lg btn-block " >
                <svg xmlns="http://www.w3.org/2000/svg" width="256" height="48" fill="currentColor" class="bi bi-download" viewBox="0 0 16 16">
                    <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"/>
                    <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"/>
                </svg>
                </button> 
            </div>
        </div>

    </div>
)
}

export default PagoQR